...
# LMAC importance
def lmac_importance(y, baseline_prob, sr):
    stride = int(0.1 * sr)
    win = int(0.5 * sr)
...
def plot_and_save_all(y, sr, pred, importance, true_label, filename):
    ...
    plt.tight_layout()
    wf_path = f"{SAVE_DIR}/{fname}_waveform.png"
    plt.savefig(wf_path); st.image(wf_path); plt.close()
    ...
    plt.tight_layout()
    spec_path = f"{SAVE_DIR}/{fname}_spectrogram.png"
    plt.savefig(spec_path); st.image(spec_path); plt.close()

    # LMAC Plot
    fig3, ax3 = plt.subplots()
    t = np.linspace(0, len(importance) * 0.1, len(importance))
    ax3.bar(t, importance, width=0.09, color='red')
    ax3.set_title("LMAC Importance (%)")
    ax3.set_xlabel("Time (s)")
    plt.tight_layout()
    lmac_path = f"{SAVE_DIR}/{fname}_lmac.png"
    plt.savefig(lmac_path); st.image(lmac_path); plt.close()

    # 🔊 Masked audio: keep only segments ≥ 60% of max
    stride = int(0.1 * sr)
    win = int(0.5 * sr)
    ...
    write(f"{SAVE_DIR}/{fname}_masked.wav", sr, masked_audio.astype(np.float32))

# --- STREAMLIT UI ---
st.title("🎤 Real-Time Emotion Detection App with LMAC")
...
if option == "Upload File":
    ...
    if uploaded_file:
        ...
        importance = lmac_importance(y, prob, sr)
